...
2026년 6월부터 KT가 로그인에 PASS/문자 본인인증을 요구합니다. 다행히 인증은 **계정 단위로 기억**되어, 어떤 방법으로든 1회만 인증하면 이후에는 도구가 인증 없이 자동 로그인됩니다. 두 가지 방법 중 편한 쪽으로:

- **일반 브라우저에서**: [speed.kt.com](https://speed.kt.com)에 로그인해 본인인증을 완료 (가장 간단)
- **도구의 디버그 모드에서**:
  ```bash
  npx -y damn-my-slow-kt@latest run --debug --dry-run --force
  ```
  열린 브라우저에서 인증을 완료하면 도구가 감지하고 측정을 이어갑니다. 터미널에 **"추가 인증 완료"**가 출력되면 목적은 달성된 것이므로, 25분짜리 측정을 기다리지 않고 `Ctrl+C`로 종료해도 됩니다.

headless 실행 중에 인증 화면을 만나면 도구가 위 안내와 함께 명확한 에러로 알려줍니다. KT가 나중에 재인증을 요구하면 같은 안내가 다시 출력되며, 그때 한 번 더 인증하면 됩니다.
