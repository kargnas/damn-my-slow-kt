...
### 추가 본인인증(PASS/문자)이 뜬다면 ([#13](https://github.com/kargnas/damn-my-slow-kt/issues/13))

2026년 6월부터 KT가 새 브라우저 세션의 로그인에 PASS/문자 본인인증을 요구합니다. 디버그 모드로 1회만 인증하면 됩니다:

```bash
npx -y damn-my-slow-kt@latest run --debug --dry-run --force
```

열린 브라우저에서 인증을 완료하면 로그인 세션이 `~/.damn-my-slow-isp/session-kt.json`에 저장되고, 이후에는 headless로 인증 없이 자동 실행됩니다. 터미널에 **"추가 인증 완료 — 세션 저장됨"**이 출력되면 세션은 이미 저장된 것이므로, 25분짜리 측정을 끝까지 기다릴 필요 없이 `Ctrl+C`로 종료해도 됩니다. KT가 세션을 만료시키면 같은 안내가 다시 출력되며, 그때 한 번 더 인증하면 됩니다.

참고: 인증은 계정 단위로 기억되는 것으로 보입니다. 일반 브라우저에서 KT 로그인 + 본인인증을 한 번 완료하는 것만으로도 도구의 로그인이 풀리는 경우가 있으니, `--debug` 실행이 어려운 환경(원격 서버 등)에서는 이 방법을 먼저 시도해보세요.
